...
"#;
    ...
    for arg in adaptor.cur_args() {
        ...
        println!("   - {} (local_idx={}) defined in block 0", adaptor.value_name(arg), local_idx);
    }
    ...
    for inst in adaptor.block_insts(entry_block) {
        ...
        for res in &results {
            ...
            println!("       -> defines {} (local_idx={})", adaptor.value_name(*res), local_idx);
        }
        ...
        for op in &operands {
            ...
            println!("       -> uses {} (local_idx={})", adaptor.value_name(*op), local_idx);
        }
    }
    ...
    if blocks.len() > 1 {
        ...
        for phi in adaptor.block_phis(loop_block) {
            ...
            println!("     - {} (local_idx={}, op_begin={}, op_count={}, op_end={})", 
            ...
            println!("       trying to access indices {} to {}", phi_info.op_begin_idx, phi_info.op_end_idx);
