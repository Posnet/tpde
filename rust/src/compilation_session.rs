...
impl<'arena> CompilationSession<'arena> {
    ...
    pub fn set_value_location(&self, value_idx: usize, location: ValueLocation) {
        self.value_locations.borrow_mut().insert(value_idx, location);
    }
    ...
    pub fn record_instruction_compiled(&self, opcode: &str) {
        ...
        *stats.instruction_counts.entry(opcode.to_string()).or_insert(0) += 1;
    }
...
mod tests {
    ...
    fn test_value_tracking() {
        ...
        assert_eq!(session.get_value_location(0), Some(ValueLocation::Register(3)));
        assert_eq!(session.get_value_location(1), Some(ValueLocation::Stack(-8)));
