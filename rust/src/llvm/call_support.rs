...
pub fn get_param_attributes(call_site: CallSiteValue, param_idx: u32) -> ArgAttribute {
    ...
    if let Some(_byval_attr) =
        call_site.get_enum_attribute(AttributeLoc::Param(param_idx), byval_kind_id)
    {
        ...
        let align = if let Some(align_attr) =
            call_site.get_enum_attribute(AttributeLoc::Param(param_idx), align_kind_id)
        {
    ...
    if call_site
        .get_enum_attribute(AttributeLoc::Param(param_idx), sret_kind_id)
        .is_some()
    {
    ...
    if call_site
        .get_enum_attribute(AttributeLoc::Param(param_idx), zext_kind_id)
        .is_some()
    {
    ...
    if call_site
        .get_enum_attribute(AttributeLoc::Param(param_idx), sext_kind_id)
        .is_some()
    {
...
pub fn get_byval_size(
    _call_site: CallSiteValue,
    _param_idx: u32,
    arg_value: inkwell::values::BasicValueEnum,
) -> u32 {
...
pub fn setup_varargs_call(
    cc_assigner: &mut crate::x64::calling_convention::SysVAssigner,
    varargs_start: u32,
) {
