...
fn create_module_with_calls(context: &Context) -> inkwell::module::Module {
    ...
    let call_result = builder
        .build_call(add_function, &[x.into(), y.into()], "result")
        .unwrap();
...
fn create_factorial_module(context: &Context) -> inkwell::module::Module {
    ...
    let cond = builder
        ...
        .unwrap();
    builder
        .build_conditional_branch(cond, base_case, recursive_case)
        .unwrap();
    ...
    let call_result = builder
        .build_call(fact_function, &[n_minus_1.into()], "call_result")
        .unwrap();
    let recursive_result = call_result
        .try_as_basic_value()
        .left()
        .unwrap()
        .into_int_value();
    let product = builder
        .build_int_mul(n, recursive_result, "product")
        .unwrap();
...
fn test_simple_function_call() {
    ...
    assert!(
        stats.total_calls > 0,
        "Should have recorded at least one call"
    );
...
fn test_recursive_function_call() {
    ...
    assert!(
        stats.total_calls > 0,
        "Should have recorded at least one recursive call"
    );
    assert!(stats.instructions_compiled > 0);
...
fn test_multiple_arguments() {
    ...
    let args: Vec<_> = (1..=8)
        .map(|i| i32_type.const_int(i, false).into())
        .collect();
