...
fn create_loop_with_phi(context: &Context) -> inkwell::module::Module {
    ...
    let sum_next = builder.build_int_add(sum_value, i_value, "sum_next").unwrap();
    
    // Loop condition
    let done = builder.build_int_compare(IntPredicate::SGE, i_next, n_param, "done").unwrap();
    builder.build_conditional_branch(done, exit_block, loop_block).unwrap();
...
fn create_swap_phi_test(context: &Context) -> inkwell::module::Module {
    ...
    builder.position_at_end(entry_block);
    builder.build_conditional_branch(cond_param, swap_block, no_swap_block).unwrap();
...
fn test_simple_loop_phi_resolution() {
    ...
    assert!(stats.phi_nodes_resolved > 0, "Expected PHI nodes to be resolved");
    
    println!("✅ Loop PHI resolution test passed!");
    println!("   Resolved {} PHI nodes", stats.phi_nodes_resolved);
    println!("   Generated {} bytes of code", compiled_functions["sum_to_n"].code_size);
}
...
fn test_multiple_predecessors_phi() {
    ...
    let cond = builder.build_int_compare(IntPredicate::SGT, n, five, "cond").unwrap();
