...
fn create_phi_test_module(context: &Context) -> inkwell::module::Module {
    ...
    let cond = builder.build_int_compare(IntPredicate::SGT, a, b, "cmp").unwrap();
...
fn create_loop_phi_module(context: &Context) -> inkwell::module::Module {
    ...
    let cond = builder.build_int_compare(IntPredicate::SLT, i_val, n, "cmp").unwrap();
...
fn test_simple_phi_compilation() {
    ...
    compiler.compile_function_by_name("max")
...
fn test_loop_phi_compilation() {
    ...
    compiler.compile_function_by_name("sum_to_n")
    ...
    // Check statistics
    let stats = compiler.session().stats();
    assert_eq!(stats.instruction_counts["Phi"], 2, "Should have 2 PHI nodes");
    assert!(stats.instruction_counts["Add"] >= 2, "Should have at least 2 add instructions");
...
fn test_phi_cycle_compilation() {
    ...
    let cond = builder.build_int_compare(IntPredicate::SGT, sum, ten, "cond").unwrap();
    builder.build_conditional_branch(cond, exit, loop_block).unwrap();
    ...
    compiler.compile_function_by_name("phi_cycle")
