...
fn create_logical_shift_right_test(context: &Context) -> inkwell::module::Module {
    ...
    let value = function.get_nth_param(0).unwrap().into_int_value();
    let shift = function.get_nth_param(1).unwrap().into_int_value();
    let result = builder.build_right_shift(value, shift, false, "result").unwrap();
    builder.build_return(Some(&result)).unwrap();
...
fn create_arithmetic_shift_right_test(context: &Context) -> inkwell::module::Module {
    ...
    let value = function.get_nth_param(0).unwrap().into_int_value();
    let shift = function.get_nth_param(1).unwrap().into_int_value();
    let result = builder.build_right_shift(value, shift, true, "result").unwrap();
    builder.build_return(Some(&result)).unwrap();
...
fn create_complex_shift_test(context: &Context) -> inkwell::module::Module {
    ...
    let right_count = builder.build_int_sub(thirty_two, count, "right_count").unwrap();
    
    // right = value >>> right_count
    let right = builder.build_right_shift(value, right_count, false, "right").unwrap();
...
fn test_shift_left_compilation() {
    ...
    println!("   Compiled {} SHL instructions", stats.instruction_counts["Shl"]);
}
...
fn test_logical_shift_right_compilation() {
    ...
    println!("   Compiled {} LSHR instructions", stats.instruction_counts["LShr"]);
}
...
fn test_arithmetic_shift_right_compilation() {
    ...
    println!("   Compiled {} ASHR instructions", stats.instruction_counts["AShr"]);
}
...
fn test_shift_with_constants() {
    ...
    let result = builder.build_left_shift(value, shift_amount, "result").unwrap();
    builder.build_return(Some(&result)).unwrap();
    ...
    // Compile the function
    compiler.compile_function_by_name("shift_by_constant").unwrap();
