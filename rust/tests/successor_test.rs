...
fn create_control_flow_module(context: &Context) -> inkwell::module::Module {
    ...
    let cmp_gt_10 = builder
        .build_int_compare(IntPredicate::SGT, x, ten, "cmp.gt.10")
        .unwrap();
    builder
        .build_conditional_branch(cmp_gt_10, if_gt_10, switch_block)
        .unwrap();

    // if.gt.10 block
    builder.position_at_end(if_gt_10);
    let twenty = i32_type.const_int(20, false);
    let cmp_gt_20 = builder
        .build_int_compare(IntPredicate::SGT, x, twenty, "cmp.gt.20")
        .unwrap();
    builder
        .build_conditional_branch(cmp_gt_20, if_gt_20, else_20)
        .unwrap();
...
fn test_block_successor_extraction() {
    ...
    assert_eq!(
        entry_successors.len(),
        2,
        "Entry block should have 2 successors"
    );
    assert!(
        analysis.has_conditional_branch(0),
    ...
    assert_eq!(
        if_gt_10_successors.len(),
        2,
        "if.gt.10 block should have 2 successors"
    );
    assert!(
        analysis.has_conditional_branch(1),
        "if.gt.10 block should have conditional branch"
    );

    // Test if.gt.20 block (should have 0 successors - return)
    let if_gt_20_successors = analysis.get_block_successors(2);
    assert_eq!(
        if_gt_20_successors.len(),
        0,
        "if.gt.20 block should have no successors"
    );
    assert!(
        !analysis.has_conditional_branch(2),
        "if.gt.20 block should not have conditional branch"
    );

    // Test switch block (should have 3 successors: case.0, case.1, default)
    let switch_successors = analysis.get_block_successors(4);
    assert_eq!(
        switch_successors.len(),
        3,
        "Switch block should have 3 successors"
    );
    assert!(
        analysis.has_conditional_branch(4),
        "Switch block should have conditional branch"
    );
    ...
    assert!(
        !analysis.is_join_block(0),
        "Entry block should not be a join block"
    );
    ...
    if let Some(idx) = switch_idx {
        ...
        assert!(
            !analysis.is_join_block(idx),
            "Switch block should not be a join block"
        );
    } else {
...
fn test_loop_successor_extraction() {
    ...
    let cond = builder
        .build_int_compare(IntPredicate::SGT, n, zero, "cond")
        .unwrap();
    builder
    ...
    assert!(
        analysis.is_join_block(1),
        "Loop header should be a join block"
    );
