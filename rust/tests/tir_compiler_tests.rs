...
fn test_analyzer_rpo() {
    ...
    let br1_func = funcs
        .iter()
        .find(|&&f| adaptor.func_link_name(f) == "br1")
        .unwrap();
...
fn test_analyzer_liveness() {
    ...
    let myfunc = funcs
        .iter()
        .find(|&&f| adaptor.func_link_name(f) == "myfunc")
        .unwrap();
    ...
    assert!(
        liveness_info.is_some(),
        "Should have liveness info for first value"
    );
}
...
fn test_analyzer_block_layout() {
    ...
    let condbr1 = funcs
        .iter()
        .find(|&&f| adaptor.func_link_name(f) == "condbr1")
        .unwrap();
    ...
    let entry_block = adaptor.entry_block();
    assert_eq!(
        layout[0], entry_block,
        "Entry block should be first in layout"
    );
}
...
fn test_parse_alloca() {
    ...
    assert_eq!(
        alignments,
        vec![1, 2, 4, 8, 16],
        "Alignments should match expected values"
    );
}
...
fn test_phi_nodes() {
    ...
    let ret_block = blocks
        .iter()
        .find(|&&b| adaptor.block_phis(b).count() > 0)
        .unwrap();
...
fn test_function_arguments() {
    ...
    let secfunc = funcs
        .iter()
        .find(|&&f| adaptor.func_link_name(f) == "secfunc")
        .unwrap();
