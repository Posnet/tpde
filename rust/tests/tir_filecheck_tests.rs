...
fn test_analyzer_rpo_filecheck() {
    ...
    let spec =
        TestSpec::parse(&contents).unwrap_or_else(|e| panic!("Failed to parse test spec: {e}"));
...
fn test_analyzer_liveness_filecheck() {
    ...
    if !path.exists() {
        println!(
            "Skipping liveness test - file not found: {}",
            path.display()
        );
        return;
    }
...
mod filecheck_internals {
    use super::*;
    use tpde::test_ir::{CheckDirective, TestSpec};
