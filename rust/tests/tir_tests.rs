...
fn test_simple_tir() {
    ...
    let phi_values: Vec<_> = ir.values.iter()
...
fn test_alloca_align_tir() {
    ...
    let allocas: Vec<_> = ir.values.iter()
    ...
    for (i, alloca) in allocas.iter().enumerate() {
        ...
        assert_eq!(align, expected_aligns[i], "Alignment mismatch for {}", alloca.name);
    }
}
...
fn test_call_tir() {
    ...
    let calls: Vec<_> = ir.values.iter()
...
"#;
    ...
    let result = TestIR::parse(tir);
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("PHI nodes must be at the beginning"));
}